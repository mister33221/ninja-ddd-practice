...
- 我們主要的思路流程大致會如下步驟
    ...
    3. 領域分析( DDD Domain Analysis )
    4. 定義限界上下文( Bounded Context )
    5. 領域模型設計( DDD Domain Model )
    6. 分層架構設計，按照 DDD 模型的分層架構來設計專案
        1. Interface Layer
        2. Application Layer
        3. Domain Layer
...
- 忍具管理：添加、更新、刪除和查詢忍具信息
- 訂單管理：創建訂單、更新訂單狀態、查詢訂單
- 忍者客戶管理：忍者註冊、登錄、更新個人信息
- 忍具包：添加忍具到忍具包、從忍具包移除忍具、查看忍具包
- 庫存管理：更新庫存、檢查庫存狀態
    ...
    2. 執行階段
        ...
        3. 定義聚合
            ...
            - 本主題我們以粉紅色便利貼來標示聚合。
                ...
                - 庫存
        4. 繪製閱讀模型與策略
            - 閱讀模型指的是標示出那些數據需要被查詢或展示，並將其添加到模型中。
            - 定義出當特定事件發生時，需要自動執行的策略。
            - 本主題我們以綠色便利貼來標示策略。
                - 訂單已創建時，發送短信通知忍者
                - 訂單狀態已更新時，發送短信通知忍者
                - 忍者已註冊時，發送郵件通知忍者
...
| 聚合 | 事件 | 命令 | 策略 |
|------|------|------|------|
| 忍具 | - 忍具已添加<br>- 忍具已更新 | - 添加忍具<br>- 更新忍具信息 | |
| 訂單 | - 訂單已創建<br>- 訂單狀態已更新 | - 創建訂單<br>- 更新訂單狀態 | - 訂單已創建時，發送短信通知忍者<br>- 訂單狀態已更新時，發送短信通知忍者 |
| 忍者 | - 忍者已註冊<br>- 忍者已登錄 | - 註冊忍者<br>- 登錄 | - 忍者已註冊時，發送郵件通知忍者 |
